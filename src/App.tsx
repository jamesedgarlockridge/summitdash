...
const Icons = {
  ...
  CloudSimple: ({ size = 24, color = 'currentColor' }) => (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      ...
      strokeLinejoin="round"
    >
      <path d="M6.82 17.3a4.5 4.5 0 0 1-1.32-8.8 5 5 0 0 1 9.5-2.2 4 4 0 0 1 5 3.7 4 4 0 0 1-4 4H6.82Z" />
    </svg>
  ),
  Rocket: ({ size = 24, color = 'currentColor' }) => (
    ...
    >
      ...
      <path d="M15 20s-1 .5-4 1c0-2 .5-3 .5-3" />
    </svg>
  ),
  Satellite: ({ size = 24, color = 'currentColor' }) => (
    ...
    >
      ...
      <path d="m20 12-2.5-2.5" />
      <path d="m12 20-2.5-2.5" />
      <path d="m9 15 3-3" />
      <path d="m11 17 3-3" />
    </svg>
  ),
  ...
  ThermometerSnowflake: ({ size = 24, color = 'currentColor' }) => (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
...
const MoonGraphic = ({ pos }: { pos: number }) => {
  ...
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <path
        d={`M 12 ${12 - radius} A ${radius} ${radius} 0 0 ${sweep} 12 ${
          12 + radius
        } A ${shadowX} ${radius} 0 0 ${1 - sweep} 12 ${12 - radius}`}
        fill="#FFFFFF"
      />
    </svg>
  );
...
export default function App() {
  ...
  const sunset = useMemo(
    () => calculateSellsSunset(selectedDate),
    [selectedDate]
  );
  const nightfall = useMemo(
    () => calculateNightfall(selectedDate),
    [selectedDate]
  );
  ...
  useEffect(() => {
    ...
    async function fetchWeather() {
      ...
      try {
        ...
        if (!pointsData.properties?.forecastGridData)
          throw new Error('Could not resolve Grid');
        ...
        const parseDuration = (durationStr: string) => {
          ...
          const timePart = durationStr.includes('T')
            ? durationStr.split('T')[1]
            : '';
        ...
        targetHours.forEach((hour) => {
          ...
          if (data.properties.skyCover?.values) {
            const val = getValueForTime(
              data.properties.skyCover.values,
              targetMs
            );
        ...
        if (windowTempsF.length > 0 && active) {
          ...
          if (cMax > 60) {
            status = 'Poor Observation';
            color = BRAND.status.poor;
            detail = 'Heavy Cloud Cover';
          } else if (cMax > 30 || wMax > 20) {
            status = 'Marginal';
            color = BRAND.status.marginal;
            detail = wMax > 20 ? 'High Winds' : 'Scattered Clouds';
          } else if (cMax > 10) {
            status = 'Fair';
            color = BRAND.status.fair;
            detail = 'High Thin Clouds';
          }
          ...
          setWeather({
            ...
            status: 'Out of Range',
            color: BRAND.status.error,
            detail: 'NWS forecast limit reached',
          });
        }
      } catch (e) {
        if (active)
  ...
  useEffect(() => {
    ...
    async function executeScrape() {
      ...
      const results = {
        ...
        rocket: {
          time: 'None Tonight',
          note: 'No Vandenberg launch scheduled',
        },
      };
        ...
        const scrapePasses = async (satId: number) => {
          try {
            ...
            const rows = doc.querySelectorAll(
              '.standardTable tbody tr.clickableRow'
            );
            for (let i = 0; i < rows.length; i++) {
              ...
              if (
                cells.length > 5 &&
                cells[0].textContent?.includes(haDateStr)
              ) {
                const mag = cells[1].textContent?.trim() || '';
                const time24 = cells[2].textContent?.trim() || '';
                let [hh_str, mm] = time24.split(':');
                let hh = parseInt(hh_str, 10);
                if (hh >= 18 && hh <= 22) {
                  const ampm = hh >= 12 ? 'PM' : 'AM';
                  const hh12 = hh % 12 || 12;
                  return {
                    time: `${hh12}:${mm} ${ampm}`,
                    note: `Mag ${mag} (Confirmed)`,
                  };
                }
              }
            }
        ...
        const scrapeSFN = async () => {
          try {
            ...
            for (let i = 0; i < datenames.length; i++) {
              ...
              if (
                (textContent.includes(sfnMonthStr) ||
                  textContent.includes(fullMonthStr)) &&
                textContent.includes(day.toString())
              ) {
                const block = el.parentElement;
                if (block && block.textContent?.includes('Vandenberg')) {
                  const mission =
                    block.querySelector('.mission')?.textContent ||
                    'Launch Scheduled';
                  const launchData =
                    block.querySelector('.launchdata')?.textContent || '';
                  const timeMatch = launchData.match(
                    /(\d+(?::\d+)?\s*[a|p]\.?m\.?)/i
                  );
                  return {
                    time: timeMatch ? timeMatch[0].toUpperCase() : 'Scheduled',
                    note: `${mission.substring(0, 25)} (SFN)`,
                  };
                }
              }
            }
  ...
  return (
    <div
      className="min-h-screen p-4 md:p-8 font-sans selection:bg-[#4B9CD3]/30"
      style={{ backgroundColor: BRAND.bgApp }}
    >
      <div className="max-w-4xl mx-auto flex flex-col min-h-[90vh]">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-[#163A58] border border-[#2B5D82] w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
              <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
                <h2 className="text-xl font-black uppercase tracking-widest text-[#75D1F5]">
                  Telemetry Sources & Failsafes
                </h2>
                <button
              ...
              <div className="p-6 overflow-y-auto space-y-6 text-sm">
                <div>
                  <h3 className="text-[#10B981] font-black uppercase text-xs mb-2 tracking-widest">
                    Program Conditions
                  </h3>
                  <p className="text-gray-300 leading-relaxed uppercase font-medium tracking-wide text-[11px]">
                    Sourced from NOAA National Weather Service (NWS) Grid APIs
                    using Kitt Peak's precise GPS coordinates. Failsafe: System
                    employs an exponential backoff retry logic and hard-coded
                    mathematical fallbacks to ensure dashboard stability during
                    API outages.
                  </p>
                </div>
                <div>
                  <h3 className="text-[#4B9CD3] font-black uppercase text-xs mb-2 tracking-widest">
                    Astronomical Calculations
                  </h3>
                  <p className="text-gray-300 leading-relaxed uppercase font-medium tracking-wide text-[11px]">
                    Sunset, Nightfall, and Moon Phase data are generated via
                    internal high-precision astronomical algorithms calibrated
                    for 31.78° N. Failsafe: Zero external dependencies; these
                    values calculate correctly 100% of the time without an
                    internet connection.
                  </p>
                </div>
                <div>
                  <h3 className="text-[#F59E0B] font-black uppercase text-xs mb-2 tracking-widest">
                    Satellite Telemetry
                  </h3>
                  <p className="text-gray-300 leading-relaxed uppercase font-medium tracking-wide text-[11px]">
                    ISS and Tiangong overflights are scraped live from
                    Heavens-Above DOM tables. Failsafe: Employs a triple-proxy
                    failover system (AllOrigins, CodeTabs, CorsProxy) to bypass
                    CORS restrictions and regional network blocks.
                  </p>
                </div>
                <div>
                  <h3 className="text-[#FF5F1F] font-black uppercase text-xs mb-2 tracking-widest">
                    Rocket Launch Data
                  </h3>
                  <p className="text-gray-300 leading-relaxed uppercase font-medium tracking-wide text-[11px]">
                    SpaceFlightNow schedule tables are parsed for
                    Vandenberg-specific mission strings. Failsafe: Real-time
                    parsing ensures immediate updates for T-minus delays or
                    scrubbed missions that static schedules miss.
                  </p>
                </div>

                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                    Kitt Peak VC Dashboard created by James Edgar Lockridge with
                    Gemini Canvas, 2026.
                  </p>
                </div>
              </div>
              <div className="p-4 bg-black/40 text-center">
                <p className="text-[10px] text-[#4B9CD3] font-black uppercase tracking-[0.2em]">
                  Operational Integrity Protocol Active
                </p>
              </div>
            </div>
          </div>
        ...
        <header className="mb-10 flex flex-col md:flex-row justify-between items-end border-b pb-6 border-white/10">
          ...
          <div className="text-right opacity-60 w-full md:w-auto mt-6 md:mt-0 text-gray-400">
            <p className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 justify-end text-gray-400">
              <Icons.MapPin size={12} /> 31.7801° N, 111.5730° W
            </p>
            <p className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 justify-end mt-1.5 text-gray-400">
        ...
        <div
          ...
          style={{
            ...
            boxShadow: `0 20px 50px -12px ${weather.color}44`,
          }}
        >
          {loading.weather && (
            <div className="absolute inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-10">
              <div className="font-bold uppercase tracking-widest text-sm animate-pulse text-[#05070A]">
                Synchronizing NWS Grid...
              </div>
            </div>
          )}
          ...
          <div className="text-center md:text-left relative z-0 pl-4 md:pl-0">
            ...
            <p className="text-sm font-bold uppercase opacity-80 mt-2">
              {weather.detail}
            </p>
          </div>
          <div className="flex gap-4 w-full md:w-auto relative z-0">
            <div className="bg-black/10 p-5 rounded-2xl flex-1 md:min-w-[140px] text-center border border-black/5 backdrop-blur-md">
              <p className="text-[9px] font-black uppercase opacity-60 mb-2">
                Sunset (Local)
              </p>
            ...
            <div className="bg-black/10 p-5 rounded-2xl flex-1 md:min-w-[140px] text-center border border-black/5 backdrop-blur-md">
              <p className="text-[9px] font-black uppercase opacity-60 mb-2 flex justify-center items-center gap-1">
                Program Low <Icons.ThermometerSnowflake size={10} />
              </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <section
            className="p-8 rounded-[2rem] space-y-8 flex flex-col shadow-lg border text-white"
            style={{ backgroundColor: BRAND.navy, borderColor: BRAND.slate }}
          >
            <h3
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className="text-xl font-black uppercase tracking-tight leading-none mb-1">
                  {moon.name}
                </p>
                <p className="text-[10px] font-bold uppercase opacity-80 text-gray-400">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[9px] font-bold uppercase opacity-60 text-gray-400">
                    Max Wind
                  </p>
              ...
              <div className="flex items-center gap-4">
                <IconBox icon={Icons.CloudSimple} />
                <div>
                  <p className="text-[9px] font-bold uppercase opacity-60 text-gray-400">
                    Cloud Cover
                  </p>
            ...
            >
              ...
              <div>
                <p className="text-[9px] font-bold uppercase opacity-60 text-gray-400">
                  Astronomical Twilight (Nightfall)
                </p>
            ...
            <div
              className="mt-auto p-4 rounded-xl flex gap-3 items-start bg-black/20 border"
              style={{ borderColor: BRAND.slate }}
            >
              <Icons.Info size={14} color={BRAND.cyan} />
              <p className="text-[10px] font-medium opacity-90 leading-relaxed uppercase tracking-wider text-gray-300">
                Data verified from official NWS Graphical Forecast matching KPNO
                coordinates.
              </p>
            </div>
          </section>
          ...
          <section className="space-y-4">
            ...
            {(Object.entries(transients) as any[]).map(([id, data]) => {
              const config: any = {
                iss: {
                  label: 'INTL. SPACE STATION PASS',
                  icon: Icons.Satellite,
                  link: 'https://www.astroviewer.net/iss/en/observation.php',
                },
                rocket: {
                  label: 'VANDENBERG LAUNCH',
                  icon: Icons.Rocket,
                ...
                tiangong: {
                  label: 'Tiangong Pass',
                  icon: Icons.Satellite,
                  link: 'https://www.astroviewer.net/iss/en/observation-css.php',
                },
              };
              const ev = config[id];
              if (!ev) return null;
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block p-5 rounded-2xl flex justify-between items-center shadow-md transition-all border text-white hover:bg-black/10 group"
                  style={{
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      ...
                      {!isNoneTonight &&
                        ...
                        !loading.transients && (
                          <div
                            className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full border-2 shadow-lg"
                            style={{
                    ...
                    <div>
                      <h4 className="font-black text-lg uppercase leading-none mb-1.5">
                        {ev.label}
                      </h4>
                  ...
                  <div className="text-right flex flex-col items-end">
                    ...
                    {!loading.transients &&
                      ...
                      data?.time !== 'Error' && (
                        <div
                          className="text-[8px] font-bold uppercase flex items-center gap-1 justify-end mt-1"
                          style={{ color: BRAND.cyan }}
                        >
                          Confirmed{' '}
                          <Icons.ExternalLink size={8} color={BRAND.cyan} />
                        </div>
                      )}
                  </div>
                </a>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex flex-col md:flex-row gap-x-8 gap-y-2 text-center md:text-left">
              <p
                ...
                style={{ color: BRAND.daygloOrange }}
              >
              ...
              <p
                ...
                style={{ color: BRAND.daygloOrange }}
              >
                KPVC Operations Manager:{' '}
                <span className="ml-1">(905) 885-9471</span>
              </p>
            </div>
